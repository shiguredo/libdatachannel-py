...
class OpenH264VideoEncoder : public VideoEncoder {
 public:
  OpenH264VideoEncoder(std::string openh264) {
  ...
  bool Init(const Settings& settings) override {
    ...
    encoder_params.iTargetBitrate = settings.bitrate;
    ...
    PLOG_INFO << "OpenH264 version is " << OPENH264_MAJOR << "."
              << OPENH264_MINOR;
    ...
    if (encoder_->InitializeExt(&encoder_params) != 0) {
      PLOG_ERROR << "Failed to initialize OpenH264 encoder";
      Release();
      //ReportError();
      return false;
    }
  ...
  void Encode(const VideoFrame& frame) override {
    if (frame.format != ImageFormat::I420) {
      PLOG_WARNING << "OpenH264 only support I420 format.";
      return;
    }
    ...
    pic.pData[0] = frame.i420_buffer->y.data();
    pic.pData[1] = frame.i420_buffer->u.data();
    pic.pData[2] = frame.i420_buffer->v.data();
    ...
    for (int i = 0; i < info.iLayerNum; ++i) {
      ...
      memcpy(encoded.data.data() + offset, layer.pBsBuf, n);
      offset += n;
    }
...
std::shared_ptr<VideoEncoder> CreateOpenH264VideoEncoder(std::string openh264) {
  return std::make_shared<OpenH264VideoEncoder>(std::move(openh264));
}