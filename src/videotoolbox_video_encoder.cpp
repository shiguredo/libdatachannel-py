...
class VideoToolboxVideoEncoder : public VideoEncoder {
 public:
  VideoToolboxVideoEncoder() {}
  ~VideoToolboxVideoEncoder() override { Release(); }
  ...
  void Encode(const VideoFrame& frame) override {
    if (frame.nv12_buffer == nullptr) {
      PLOG_ERROR << "VideoToolboxVideoEncoder only support NV12 buffer.";
      return;
    }
  ...
  void OnEncode(OSStatus status,
    ...
    // CMSampleBufferRef を encoded.data に詰める
    {
      ...
      if (key_frame) {
        ...
        encoded.data = CreateVideoBuffer(block_buffer_size);
        dst = encoded.data.data();
      }
...
std::shared_ptr<VideoEncoder> CreateVideoToolboxVideoEncoder() {
  return std::make_shared<VideoToolboxVideoEncoder>();
}
