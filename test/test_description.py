...
def test_create_description_offer():
    desc = libdatachannel.Description("v=0...", libdatachannel.Description.Type.Offer)
    assert desc.type() == libdatachannel.Description.Type.Offer
    assert isinstance(desc.bundle_mid(), str)


def test_add_audio_track():
    desc = libdatachannel.Description("v=0...", libdatachannel.Description.Type.Offer)
    index = desc.add_audio("audio", libdatachannel.Description.Direction.SendOnly)
    assert index == 0
    assert desc.has_audio_or_video()
    assert desc.media_count() == 1

    media = desc.media(0)
    assert isinstance(media, libdatachannel.Media)

    media.as_audio().add_opus_codec(111)


def test_add_application_track():
    desc = libdatachannel.Description("v=0...")
    index = desc.add_application("data")
    assert index == 0

    app = desc.application()
    assert isinstance(app, libdatachannel.Application)
...
def test_extmap_operations():
    app = libdatachannel.Application()
    ext = libdatachannel.Entry.ExtMap(1, "urn:ietf:params:rtp-hdrext:sdes:mid")
    app.add_ext_map(ext)
...
def test_certificate_fingerprint_operations():
    fp = libdatachannel.CertificateFingerprint()
    fp.algorithm = libdatachannel.CertificateFingerprint.Algorithm.Sha256
    fp.value = "AB:CD:EF"

    assert fp.is_valid() in (True, False)  # Depending on implementation
    id_str = libdatachannel.CertificateFingerprint.algorithm_identifier(fp.algorithm)
    size = libdatachannel.CertificateFingerprint.algorithm_size(fp.algorithm)
